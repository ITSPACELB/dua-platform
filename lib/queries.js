...
// تحديث التوعية
export async function updateAwareness(content, links, isActive) {
...
// إضافة كتاب
export async function addBook(title, url) {
  const text = `
    INSERT INTO library (title, url)
    VALUES ($1, $2)
    RETURNING *
  `
  const result = await query(text, [title, url])
  return result.rows[0]
}
